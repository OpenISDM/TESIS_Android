...
public class myEarthquakeListAdapter extends BaseAdapter implements Filterable {
	private int[] mTo;
...
//		if (Integer.parseInt((String) dataSet.get("No")) > lastID) {
//			view.findViewById(R.id.textViewNew);
			...
			if (v != null) {
				...
				if (!bound) {
					if (v instanceof Checkable) {
						...
						// CheckBoxes).
//						setViewText((TextView) v, text);
						// TODO add my implement here
						if (v.getId() == R.id.eqlist_ml) { // Note: ML
						// Log.d("Here!",
						// "In myEarthquakeListadapter ML:"+Double.parseDouble(text));
							setViewText((TextView) v, text);
							if (Double.parseDouble(text) < 3.5) {
								((TextView) v).setTextColor(mContext
										.getResources().getColor(R.color.gray));
							} else if (Double.parseDouble(text) >= 3.5
									&& Double.parseDouble(text) < 5.0) {
								((TextView) v).setTextColor(mContext
										.getResources()
...
//							Log.d("myLog", "In EQListAdapter eqlist new :" + text);
							...
							// ((TextView) v).setPaintFlags(((TextView)
							// v).getPaintFlags() | Paint.UNDERLINE_TEXT_FLAG);
